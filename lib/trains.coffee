...
update = ->
  $("#locationdisplay").html if cs["subway"] and not cs["train"] then "#{cs()["name"]} [Subway]" else if cs["train"] and not cs["subway"] then "#{cs()["name"]} [Train]" else if cs["train"] and cs["subway"] then "#{cs()["name"]} [Subway/Train]"
